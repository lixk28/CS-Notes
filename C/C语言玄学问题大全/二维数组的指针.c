...
//     表示形式                             含义                    值(地址)   //
//        a                        二维数组名，指向一维数组a[0]        A       //
//  a[0]  *(a+0)  *a                    0行0列元素的地址               A       //
//     a+i  &a[i]                         i行起始地址               A+i*n*B    //
//    a[i]  *(a+i)                      i行首元素的地址             A+i*n*B    // 
//  a[i]+j  *(a+i)+j  &a[i][j]           a[i][j]的地址             A+(i*n+j)*B //
// *(a[i]+j)  *(*(a+i)+j)  a[i][j]        a[i][j]的值                a[i][j]   //
/*******************************************************************************/ 
// 注意:以上中 i行起始地址 和 i行首元素的地址 实际上是不同的
